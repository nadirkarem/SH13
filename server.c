...
int main(int argc, char *argv[]){
    ...
    while (1){
        ...
        if (fsmServer==0) {
			...
			{
				case 'G': {
					int idSuspect, suspect;
					sscanf(buffer+2, "%d %d", &idSuspect, &suspect);
					// Vérifier et annoncer le résultat
					if (suspect == deck[12]) {
						printf("%s a gagne\n", tcpClients[idSuspect].name);
					} else {
						printf("%s, tu as faux\n", tcpClients[idSuspect].name);
					}
				...
				case 'O': {
					int idWhoAsked, objet;
					sscanf(buffer+2, "%d %d", &idWhoAsked, &objet);
					// Chacun des autres joueurs répond
					for (int i = 0; i < 4; i++) {
						if (i == idWhoAsked) continue;
						int valeur = (tableCartes[i][objet] > 0) ? 100 : 0;
						sprintf(reply, "V %d %d %d", i, objet, valeur);
						broadcastMessage(reply);
					}
					// Passer au joueur suivant
					joueurCourant = (joueurCourant + 1) % 4;
				...
				case 'S': {
					int idWhoAsked, cible, objet;
					sscanf(buffer+2, "%d %d %d", &idWhoAsked, &cible, &objet);
					// Réponse ciblée
