    ...
    while (!quit) {
        ...
        if (synchro==1) {
			...
			switch (gbuffer[0]) {
				...
				case 'D':
					// Message 'D' : distribution initiale des 3 cartes
					// format "D c0 c1 c2"
					sscanf(gbuffer, "D %d %d %d",
						&b[0],
						&b[1],
						&b[2]);
					break;
				...
				case 'M': {
					// Message 'M' : numéro du joueur courant
					// format "M <joueur>"
					int current;
					sscanf(gbuffer, "M %d", &current);
					// n’autorise Go que si c’est ton tour
				...
				case 'V': {
					// Message 'V' :
					// soit réponse à O/S : "V id objet valeur"
					// soit distribution tableau : "V v0 v1 … v7"
					int id2, obj2, val, n;
					// on essaie d’abord le parsing id/objet/valeur
					n = sscanf(gbuffer + 2, "%d %d %d", &id2, &obj2, &val);
					if (n == 3) {
						// rempli une case unique
						tableCartes[id2][obj2] = val;
					} else {
						// sinon on lit 8 valeurs pour ta ligne gId
						char *p = gbuffer + 2;
						for (int i = 0; i < 8; i++) {
							int v, read;
							sscanf(p, "%d%n", &v, &read);
